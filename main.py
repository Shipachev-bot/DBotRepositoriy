...
async def offline_maps(message: types.Message, state: FSMContext):
    await message.answer_photo(
        ...
        caption='<b>Чтобы использовать карты без интернета - нужно их сохранить в разделе офлайн карты</b>\n\nНажмите картинку (+) справа от заголовка и вам откроется экран создания области \nДобавьте две точки на карту так, чтобы попадала в красный прямоугольник интересующая вас область.',
        parse_mode="html")
    await message.answer_photo(
        photo='AgACAgIAAxkBAAINcmdF2t_dGU6E_CxTgV9BmCJKt1qXAAKZ6DEbhOYwSlycucBJjmGPAQADAgADeQADNgQ',
        caption='Выберите слой для загрузки и нажмите «Загрузить» \n',
        parse_mode="html")
    await message.answer_photo(
